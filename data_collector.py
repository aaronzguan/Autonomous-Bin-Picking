...
if __name__ == "__main__":
    ...
    for i in range(num_data):
        ...
        object1_pose = objs['Shape'][1][:3]
        object2_pose = objs['Shape1'][1][:3]
        object3_pose = objs['Shape3'][1][:3]
        pose_data.append(np.hstack([object1_pose, object2_pose, object3_pose]))
        rgb = obs.wrist_rgb
        depth = obs.wrist_depth
        mask = obs.wrist_mask
        cv2.imwrite('dataset/training/wrist_rgb_{}.png'.format(i+ base_data), rgb*255)

    pose_data = np.asarray(pose_data)
    print(pose_data.shape)
    np.save('train_pose_data_2.npy', pose_data)

    with open('dataset/train_data.txt', 'a') as f:
        for i in range(num_data):
            f.write('training/wrist_rgb_{}.png\n'.format(i+ base_data))
        ...
        # import pptk
        # v = pptk.viewer(point_cloud.reshape(-1,3))
        # cv2.imwrite('depth.png', np.stack((depth * 255,)*3, axis=-1))
